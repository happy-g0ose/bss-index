...
export default function App() {
  ...
  // Categories list in Russian dynamically compiled from dataset
        ...
        <section className="relative rounded-3xl p-8 md:p-12 overflow-hidden border border-white/5 bg-gradient-to-br from-neutral-900/85 to-[#0b0f19]/90 backdrop-blur-xl">
          ...
          <div className="relative z-10 max-w-3xl space-y-6">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-amber-500/20 bg-amber-500/5 text-[10px] font-extrabold tracking-widest uppercase text-amber-400">
              ...
              Прайс-лист предметов Bee Swarm
            </span>
            <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white uppercase leading-tight font-sans">
              Ценность Стикеров & <br />
              <span className="bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 bg-clip-text text-transparent">
                Ваучеров BSS
              </span>
            </h1>
            <p className="text-neutral-400 text-sm md:text-base max-w-xl leading-relaxed font-medium">
              Исследуйте актуальные расценки на все предметы из симулятора пчеловода. Добавляйте предметы в калькулятор сделок ниже, чтобы оценить прибыльность вашего обмена.
            </p>
          </div>
        </section>

        {/* Trade Calculator Widget */}
        <section id="trade-calculator-section" className="scroll-mt-24">
          <TradeCalculator
            sideA={sideA}
            sideB={sideB}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  type="text"
                  placeholder="Быстрый поиск стикера по названию..."
                  value={searchQuery}
              ...
              <div className="flex items-center gap-2 select-none self-end md:self-auto">
                ...
                <span className="text-xs text-neutral-400 font-bold">Сортировка:</span>
                <select
                  value={sortingOption}
                ...
                >
                  <option value="value-desc">По ценности (убыв.)</option>
                  <option value="value-asc">По ценности (возр.)</option>
                  <option value="demand-desc">По спросу (хайп)</option>
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-white/5 pb-4 select-none">
              <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mr-2 flex items-center gap-1">
                <Filter className="h-3.5 w-3.5 text-neutral-500" /> Фильтр:
              </span>
              {categories.map(cat => (
                <button
                  key={cat}
            ...
            <AnimatePresence mode="popLayout">
              {processedItems.length > 0 ? (
                ...
                >
                  ...
                  <p className="text-xs font-semibold">Предметы по вашему запросу не найдены.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
